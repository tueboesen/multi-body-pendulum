...
class MultiBodyPendulum:
    ...
    def get_angles_from_coordinates(x,y,vx,vy):
        ...
        """
        assert torch.is_floating_point(x) # We explicitly test for this since the code will run for integers, but produce nonsensible results
        assert torch.is_floating_point(y) # We explicitly test for this since the code will run for integers, but produce nonsensible results
        assert torch.is_floating_point(vx) # We explicitly test for this since the code will run for integers, but produce nonsensible results
        assert torch.is_floating_point(vy) # We explicitly test for this since the code will run for integers, but produce nonsensible results
    ...
    def get_coordinates_from_angles(thetas,dthetas):
        ...
        """
        assert torch.is_floating_point(thetas) # We explicitly test for this since the code will run for integers, but produce nonsensible results
        assert torch.is_floating_point(dthetas) # We explicitly test for this since the code will run for integers, but produce nonsensible results
        n = dthetas.shape[0]
        x = torch.empty_like(thetas)
        y = torch.empty_like(thetas)
        vx = torch.empty_like(dthetas)
    ...
    def step_slow(self, theta, dtheta):
        ...
        for i in range(n):
            tmp = 0
            for j in range(n):
                tmp = tmp - self._c(i, j) * dtheta[j] * dtheta[j] * torch.sin(theta[i] - theta[j])
    ...
    def step(self, theta, dtheta):
        ...
        B = - self._C * (dtheta[None, :] * dtheta[None, :]) * torch.sin(theta[:, None] - theta[None, :])
        b = torch.sum(B,dim=1)
    ...
    def plot_pendulum(x,y,vx=None,vy=None,fighandler=None,color_velocity='red',color_pendulums='blue',color_strings='grey',file=None):
        ...
        if vx is None or vy is None:
            assert vx == vy, "If vx or vy is not None, then both of them have to be given."
        else:
            vx = vx.numpy()
            vy = vy.numpy()
